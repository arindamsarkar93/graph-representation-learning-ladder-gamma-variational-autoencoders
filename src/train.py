...
# Train model
    ...
    if FLAGS.missing_data:
        qual_file = '../data/qual_missing_' + dataset_str + '_' + model_str + k_fold_str + FLAGS.hidden
    else:
...
def main():
    ...
    if FLAGS.semisup_train:
        placeholders = {
            ...
            'start_semisup':tf.placeholder(tf.float32)
        }
    else:
        placeholders = {
            'features': tf.sparse_placeholder(tf.float32),
    ...
    if FLAGS.use_k_fold: # Don't use k-fold for large dataset
        ...
        if FLAGS.test:
                adj_score  = load_model(placeholders, model, opt, adj_train, test_edges,
                                                    test_edges_false, features, sess)
        else:
                adj_score, model_z = train(placeholders, model, opt, adj_train, train_edges, val_edges,
