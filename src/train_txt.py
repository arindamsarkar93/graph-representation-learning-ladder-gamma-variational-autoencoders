...
from preprocessing import preprocess_graph, construct_feed_dict, sparse_to_tuple, mask_test_edges, mask_train_edges, mask_train_edges_txt
...
save_path_disk = "/data1/arindam/lgvae/data_models"
...
# Code Port from Dr. Piyush's auc_pr.m
...
# Train model
    ...
    for epoch in range(FLAGS.epochs):
        ...
        if True:#avg_accuracy > 0.9 or model_str == 'gcn_vae':
                ...
                print("Epoch:", '%03d' % (epoch + 1), "cost=", "{:.3f}".format(avg_cost), "kl=", "{:.3f}".format(outs[3]), "reg=", "{:.4f}".format(regularization), #"val_roc=", "{:.3f}".format(roc_curr), "val_ap=", "{:.3f}".format(ap_curr), 
    ...
    np.savez(qual_file,theta = theta_save, posterior_theta_params = posterior_theta_params_save, lambda_mat = lambda_mat, roc_score = roc_score, ap_score = ap_score, auc_pr = auc_pr, expr_info = FLAGS.expr_info, adj_train = adj_train, phis = phi_save)       
    #saver.restore(sess=sess, save_path=(save_dir+name))
...
def main():
    ...
    if FLAGS.use_k_fold: # Don't use k-fold for large dataset
        ...
        if FLAGS.random_split:
            adj_train, train_edges, train_edges_false = mask_train_edges_txt(adj)
        else:
    ...
    if FLAGS.log_results:
        results_log_file = '../data/results_log_nips_ss_22/' + dataset_str + '_' + model_str + k_fold_str + FLAGS.hidden + '.log'
